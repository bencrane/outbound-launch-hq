...
interface WorkflowConfig {
  ...
  source_record_array_field: string | null;  // e.g., "people" - if set, iterate over this array
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: workflowConfig, error: workflowError } = await supabase
      ...
      .select("id, workflow_slug, storage_worker_function_url, source_record_array_field")
    ...
    if (!workflowConfig.storage_worker_function_url) {
      ...
      return new Response(
        JSON.stringify({
          ...
          hint: "Set storage_worker_function_url in db_driven_enrichment_workflows"
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    const arrayFieldPath = workflowConfig.source_record_array_field;
